...
public class AmazonMovieWebCrawler {
   ...
   */
  private final static Map<String, String> AMAZON_COPY_TYPE_MAP = ConfigurationHelper.createValMap("dvdb.amazon.grabber.matchCopyType");

  /**
   * Map which contauns the amazon age rating -> internal age rating
   */
  private final static Map<String, String> AMAZON_AGE_RATING_MAP = ConfigurationHelper.createValMap("dvdb.amazon.grabber.matchAgeRating");

  /**
   * List of strings which are to remove from the title
   */
  private static List<String> AMAZON_REMOVE_FROM_TITLE = ConfigFactory.load().getStringList("dvdb.amazon.grabber.removeFromTitle");
    ...
    for (Jerry child : children) {
      ...
      for (final Jerry typeLink: typeLinks) {
        ...
        if(AMAZON_COPY_TYPE_MAP.containsKey(typeText) == false) {
          Logger.info("Skipping amazon type: "+typeText+" for asin: "+asin+" the type was not configured in the type map");
          continue;
        }
