...
public class MovieAttributeDao {
  ...
  public static Set<MovieAttribute> findAttributesByName(final Set<String> values, final EMovieAttributeType type) {
    final Set<MovieAttribute> findSet = MovieAttribute.FINDER.query()
        .where()
        .in("value", values)
  ...
  public static Set<MovieAttribute> gatherAndAddAttributes(final Set<String> attributeValues, final EMovieAttributeType type) {
    ...
    Set<MovieAttribute> dbAttributes = MovieAttributeDao.findAttributesByName(attributeValues, type);
    if (dbAttributes == null) {
      dbAttributes = new HashSet<>();
    }

    final Set<MovieAttribute> attributes = new HashSet<>();
    ...
    for (final String formAttr : attributeValues) {
      ...
      if (movieAttributeToAdd == null) {
        movieAttributeToAdd = MovieAttributeDao.createAttribute(type, trimedFormAttr);
      }
  ...
  public static String searchAvaibleAttributesAsJson(final EMovieAttributeType attributeType, final String searchTerm) {
    ...
    final List<MovieAttribute> attributes = MovieAttribute.FINDER.query()
        .where()
